...
class NoteAIService {
    ...
    private suspend fun callAIAPI(prompt: String): String = withContext(Dispatchers.IO) {
        // 优先使用通义千问
        if (SDKConfig.TONGYI_API_KEY != "your-tongyi-api-key" && SDKConfig.TONGYI_API_KEY.isNotEmpty()) {
            android.util.Log.d("NoteAIService", "使用通义千问API...")
            return@withContext callTongyiAPI(prompt)
        }
        
        // 回退到豆包API
        android.util.Log.d("NoteAIService", "使用豆包API...")
        return@withContext callDoubaoAPI(prompt)
    }
